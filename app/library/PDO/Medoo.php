...
class Medoo
{
    ...
    //PDO错误计数
    protected static $pdoErrorCounter = [];

    //PDO错误合集
    protected static $pdoErrors = [];

    /**
     * [errorCounterIncr 错误计数++]
     * @return [type] [description]
     */
    protected static function errorCounterIncr()
    {
        $instance = get_called_class();
        self::$pdoErrorCounter[$instance]++;
    }

    /**
     * [errorCounter 拉取错误计数总数]
     * @return [type] [description]
     */
    public function errorCounter()
    {
        $instance = get_called_class();
        return self::$pdoErrorCounter[$instance];
    }
    ...
    {
        return self::$pdoErrors;
    }

    /**
     * [queryValidate 执行结果校验]
     * @param  array  $error [description]
     * @return [type]        [description]
     */
    protected function queryValidate()
    {
        $error = $this->error();
        if (intval($error[0]) == 0 && $error[1] == "" && $error[2] == "") {
        } else {
            self::errorCounterIncr();
            $instance = get_called_class();
            self::$pdoErrors[$instance][] = $error;
        }
    }
    ...
    public function update($table, $data, $where = null)
    {
        ...
        $result = $this->exec('UPDATE ' . $this->tableQuote($table) . ' SET ' . implode(', ', $fields) . $this->whereClause($where));
        $this->queryValidate();
        return $result;
    }
    ...
    {
        $result = $this->exec('DELETE FROM ' . $this->tableQuote($table) . $this->whereClause($where));
        $this->queryValidate();
        return $result;
    }
    ...
    public function replace($table, $columns, $search = null, $replace = null, $where = null)
    {
        ...
        $result = $this->exec('UPDATE ' . $this->tableQuote($table) . ' SET ' . $replace_query . $this->whereClause($where));
        $this->queryValidate();
        return $result;
    }
